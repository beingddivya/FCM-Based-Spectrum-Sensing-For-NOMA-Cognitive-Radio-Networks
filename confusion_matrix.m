% Uplink NOMA spectrum senisng: Using FCM
% SM: 01-02-2024
clear;
clc;
close all;
...
MaxTrials = 1000;        % Number of trials of Monte Carlo simulations with different channels
...
data = zeros(K,M);
accu = 0;
C = zeros(4,4); % Initialize confusion matirx
...
for ep = 1:MaxTrials
    ...
    %-------------------------- FCM method --------------------------------
    % V     : fuzzy centers
    % U     : fuzzy belongingness
    options = [2 50 0.001 0];
    [V,U,objFun] = fcm(X,4,options);
    U = U';
    ...
    C = C + confusionmat(detected_state,actual_state);
end
accuracy = accu/(MaxTrials*K)
C = C/(MaxTrials*K);
confusionchart(round(C*100))
